...
public class Reply {
	...
	public static void createPlayer(String name){
		if(!(playerExists(name))){
			KitPvP.getInstance().mysql.update("INSERT INTO Reply(NAME, LAST) VALUES ('" + name + "', 'ThreshBandicoot');");
		}
	}
	...
	public static String getLast(String name){
		...
		if(playerExists(name)){
			try{
				ResultSet rs = KitPvP.getInstance().mysql.query("SELECT * FROM Reply WHERE NAME = '"
						+ name + "'");
                if ((!rs.next()));
	...
	public static void setLast(String name, String target){
		if(playerExists(name)){
			KitPvP.getInstance().mysql.update("UPDATE Reply SET LAST= '" + target + "' WHERE NAME= '"
                    + name + "';");
		} else {
			createPlayer(name);
			setLast(name, target);
