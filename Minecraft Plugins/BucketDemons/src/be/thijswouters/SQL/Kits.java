...
public class Kits {
	...
	public static void createPlayer(String name){
		if(!(playerExists(name))){
			DemonsCore.getInstance().mysql.update("INSERT INTO Kits(Player, dDefault, dZombie, dSkeleton, oDefault, oMiner, oArcher)"
	...
	public static String getKitStatus(String player, String kit){
		...
		if(playerExists(player)){
			try{
				ResultSet rs = DemonsCore.getInstance().mysql.query("SELECT " + kit + " FROM Kits WHERE Player = '"
						+ player + "'");
                if ((!rs.next()));
                i = rs.getString(kit);
			} catch (SQLException e){
	...
	public static void setKitStatus(String player, String kit, String status){
		if(playerExists(player)){
			DemonsCore.getInstance().mysql.update("UPDATE Kits SET " + kit + " = '" + status + "' WHERE Player= '"
                    + player + "';");
		} else {
			createPlayer(player);
