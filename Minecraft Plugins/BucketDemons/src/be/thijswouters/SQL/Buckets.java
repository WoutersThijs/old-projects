...
public class Buckets {
	...
	public static void createPlayer(String name){
		if(!(playerExists(name))){
			DemonsCore.getInstance().mysql.update("INSERT INTO Buckets(NAME, BUCKETS) VALUES ('" + name + "', '0');");
		}
	}
	...
	public static Integer getBuckets(String name){
		...
		if(playerExists(name)){
			try{
				ResultSet rs = DemonsCore.getInstance().mysql.query("SELECT * FROM Buckets WHERE NAME = '"
						+ name + "'");
                if ((!rs.next()));
                i = rs.getInt("BUCKETS");
			} catch (SQLException e){
	...
	public static void setBuckets(String name, Integer i){
		if(playerExists(name)){
			DemonsCore.getInstance().mysql.update("UPDATE Buckets SET BUCKETS= '" + i + "' WHERE NAME= '"
                    + name + "';");
		} else {
			createPlayer(name);
			setBuckets(name, i);
